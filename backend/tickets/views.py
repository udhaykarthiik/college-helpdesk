...
class TicketViewSet(viewsets.ModelViewSet):
    ...
    def send_websocket_notification(self, ticket):
        """Send WebSocket notification to all agents about new ticket"""
        try:
            ...
            async_to_sync(channel_layer.group_send)(
                "agents",
                {
                    ...
                    "ticket_id": ticket.id,
                    "title": ticket.title,
                    "raised_by": ticket.raised_by.user.username,
                    "priority": ticket.priority,
                    "created_at": ticket.created_at.isoformat()
                }
            )
            print(f"📡 WebSocket notification sent for ticket #{ticket.id}")
    ...
    def create(self, request, *args, **kwargs):
        ...
        # ✅ Send WebSocket notification to all agents
        self.send_websocket_notification(ticket)
    ...
    # ========== PUBLIC CONVERSATIONS ENDPOINT ==========
    @action(detail=True, methods=['get'], authentication_classes=[], permission_classes=[])
    def public_conversations(self, request, pk=None):
        """
        Public endpoint for users to view their ticket conversations (no login required)
        Only shows non-internal notes (customer-facing messages)
        """
        ticket = self.get_object()
        email = request.query_params.get('email')
        ...
        # Only show non-internal notes to customers
