...
# ============================================================
#  SHARED HELPER — broadcast a ticket to the dashboard group
# ============================================================

def _build_ticket_payload(ticket):
    """
    Return a JSON-serializable dict that matches exactly what
    AgentDashboard.js expects inside ticketUpdates[].
    """
    try:
        customer_name = (
            ticket.raised_by.user.get_full_name()
            or ticket.raised_by.user.username
        ) if ticket.raised_by and hasattr(ticket.raised_by, 'user') else 'Guest'
    except Exception:
        customer_name = 'Guest'

    try:
        customer_email = (
            ticket.raised_by.user.email
        ) if ticket.raised_by and hasattr(ticket.raised_by, 'user') else ''
    except Exception:
        customer_email = ''
...
def notify_dashboard(ticket, update_type='new_ticket'):
    """
    Broadcast a ticket event to ALL connected agent dashboards.
    Safe to call from any sync Django view — never raises.

    update_type: 'new_ticket' | 'updated' | 'new_message'
    """
    try:
        channel_layer = get_channel_layer()
        if channel_layer is None:
            print("⚠️  Channel layer is None — is CHANNEL_LAYERS configured?")
            return

        payload = _build_ticket_payload(ticket)

        async_to_sync(channel_layer.group_send)(
            "dashboard_updates",          # ← must match consumer group name
            {
                "type": "ticket_update",  # ← maps to TicketConsumer.ticket_update()
                "update_type": update_type,
                "ticket_id": ticket.id,
                "ticket": payload,
            }
        )
        print(f"📡 Dashboard notified: [{update_type}] ticket #{ticket.id}")
    except Exception as e:
        # Never let a notification failure break ticket creation
        print(f"⚠️  Dashboard notify failed: {e}")
...
class TicketViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Refresh ticket from DB so updated_at / assigned_to are current
        ticket.refresh_from_db()

        # ✅ Broadcast to agent dashboards via correct group name & payload shape
        notify_dashboard(ticket, update_type='new_ticket')
    ...
    def public_create(self, request):
        ...
        if recent_tickets >= 3:
            return Response({"error": "Too many tickets from this email. Please wait."}, status=429)
        ...
        if serializer.is_valid():
            ...
            Conversation.objects.create(
                ...
                sender_type='agent',
                message=(
                    f"Thank you for contacting support. Your ticket #{ticket.id} has been created. "
            ...
            # Refresh so assigned_to and updated_at are accurate
            ticket.refresh_from_db()
    ...
    def add_user_reply(self, request, pk=None):
        ...
        # Notify dashboard that this ticket got a new message
        ticket.refresh_from_db()
        notify_dashboard(ticket, update_type='new_message')
    ...
    def public_conversations(self, request, pk=None):
        """Public endpoint — customer view, no internal notes"""
        ticket = self.get_object()
        email = request.query_params.get('email')

        if not email or ticket.raised_by.user.email != email:
            return Response({"error": "Unauthorized - Invalid email"}, status=401)

        conversations = ticket.conversations.filter(is_internal_note=False).order_by('created_at')
    ...
    def assign(self, request, pk=None):
        ...
        try:
            ...
            ticket.refresh_from_db()
            notify_dashboard(ticket, update_type='updated')
    ...
    def add_conversation(self, request, pk=None):
        ...
        if sender == 'agent' and not is_internal:
            ...
            # Notify dashboard about new agent reply
            ticket.refresh_from_db()
            notify_dashboard(ticket, update_type='new_message')
    ...
    def quick_status_change(self, request, pk=None):
        ...
        if new_status not in dict(Ticket.STATUS_CHOICES):
            return Response(
                {"error": f"Invalid status. Choose from: {list(dict(Ticket.STATUS_CHOICES).keys())}"},
                status=400
            )
    ...
    def ai_suggest_response(self, request, pk=None):
        try:
            ...
            latest_conversation = ticket.conversations.filter(sender_type='user').last()

            if not latest_conversation:
                return Response({"error": "No user message found"}, status=400)

            suggested = generate_canned_response(ticket.title, latest_conversation.message)
...
class CannedResponseViewSet(viewsets.ModelViewSet):
    ...
    def popular(self, request):
        ...
        popular = CannedResponse.objects.filter(college_id=college_id).order_by('-usage_count')[:limit]
    ...
    def by_shortcode(self, request):
        ...
        try:
            response = CannedResponse.objects.get(shortcode=shortcode, college_id=college_id)
...
class KnowledgeArticleViewSet(viewsets.ModelViewSet):
    ...
    def popular(self, request):
        ...
        articles = KnowledgeArticle.objects.filter(
            college_id=college_id, is_published=True
