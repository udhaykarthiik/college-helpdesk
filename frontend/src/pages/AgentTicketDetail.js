...
function AgentTicketDetail() {
    ...
    useEffect(() => {
        fetchTicket();
        fetchConversations();  // ✅ ADDED - fetch conversations separately
        fetchCannedResponses();
        fetchAttachments();
        fetchAiAnalysis();
    }, [id]);
    ...
    // ✅ ADDED - fetch conversations separately
    const fetchConversations = async () => {
        try {
            console.log('🟡 Fetching conversations for ticket:', id);
    ...
    const handleAddConversation = async (e) => {
        ...
        try {
            ...
            setReply('');
            await fetchTicket();
            await fetchConversations();  // ✅ ADDED - refresh conversations after sending
        } catch (err) {
            console.error('Error sending reply:', err);
        } finally {
            setSubmitting(false);
    ...
    const handleAddInternalNote = async () => {
        ...
        try {
            ...
            setReply('');
            await fetchTicket();
            await fetchConversations();  // ✅ ADDED - refresh conversations after adding note
        } catch (err) {
            console.error('Error adding note:', err);
            alert('Failed to add internal note');
        } finally {
